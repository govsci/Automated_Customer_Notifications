...
using Automated_Customer_Notifications.API_DHL;

namespace Automated_Customer_Notifications
{
    public class Est_Delivery_Date_Change_Notifications
    {
        private List<AutoResponder> autoResponders = new List<AutoResponder>();
        public Est_Delivery_Date_Change_Notifications()
        {
            GetReady();
            SendResponders();
        }
        ...
        {
            AutoResponder auto = Database.GetResponder("108");
            List<OrderHeader> orders = Database.GetInlineData("Get Est Del Date Changes");
            ...
            foreach(OrderHeader o in orders)
            {
                ...
                if (order.Credentials != null && order.Lines.Count > 0)
                {
                    try
                    {
                        BuildXml build = new OrderConfirmation(order, "backordered");
                        order.XML = build.XML;
                    ...
                    {
                        Email.SendErrorMessage(ex, "Est_Delivery_Date_Change_Notifications", "GetReady", null);
                    }
                }
                ...
                if (order.SellToCustomerNo == "OAK RIDGE NTL LAB")
                    responder.Body = responder.Body.Replace("<p>Dear <strong>", "<p style='color:red;font-weight:bold'>Please make sure that you update the expected arrival dated in ORNL Marketplace!</p><p>Dear <strong>");
                ...
                foreach(OrderLine line in order.Lines)
                {
                    cart += responder.Cart.Replace("[SupplierPartNo]", line.VendorName + "<br>" + line.VendorItemNo)
                                ...
                                .Replace("[ReqDelDate]", line.RequestedDeliveryDate.ToShortDateString())
                                .Replace("[EstDelDate]", line.BackOrderDate.ToShortDateString());
                }
        ...
        private void SendResponders()
        {
            foreach (AutoResponder auto in autoResponders)
            {
                ...
                {
                    Email.SendErrorMessage(ex, "Est_Delivery_Date_Change_Notifications", "SendResponders", new SqlCommand(auto.EmailTo + ";" + auto.EmailCc + ";" + auto.EmailBcc));
                }
                Database.InsertAutoResponder(auto, body, "Estimated Delivery Date Change");
            }
        }
    }
}
