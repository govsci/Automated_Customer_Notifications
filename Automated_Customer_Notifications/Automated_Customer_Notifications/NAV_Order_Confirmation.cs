...
{
    public class NAV_Order_Confirmation
    {
        private List<AutoResponder> autoResponders = new List<AutoResponder>();
        public NAV_Order_Confirmation()
        {
            GetReady();
            SendResponders();
        }
        ...
        {
            AutoResponder auto = Database.GetResponder("105");
            List<OrderHeader> orders = Database.GetHeaderData("Nav Order Confirm Header");

            for (int i = 0; i < orders.Count; i++)
            {
                OrderHeader order = orders[i];
                Database.GetLineData(ref order, "Nav Order Confirm Line");
                ...
                if (order.Credentials != null && order.Lines.Count > 0 && order.Credentials.OrderConfirm == 1)
                {
                    ...
                    {
                        Email.SendErrorMessage(ex, "NAV_Order_Confirmation", "GetReady", null);
                    }
                }
                ...
                if(order.SellToCustomerName.StartsWith("EPA "))
                {
                    responder.Body.Replace("<p>Thank you for choosing Government Scientific Source!</p>"
                        , "<p>Please note that orders with multiple line items may ship from different destinations and may not arrive as a \"Ship Complete\" status. In addition, the sales order will be billed complete, unless an item is on backorder 60 days from the order date.</p><p>Thank you for choosing Government Scientific Source!</p>");
                }
        ...
        private void SendResponders()
        {
            foreach (AutoResponder auto in autoResponders)
            {
                ...
                try
                {
                    if (Email.TestEmail(auto.EmailTo, true) && Constants.DeploymentMode == "production" && auto.Order.OrderDate.Date >= check.Date)
                ...
                {
                    Email.SendErrorMessage(ex, "NAV_Order_Confirmation", "SendResponders", new SqlCommand(auto.EmailTo + ";" + auto.EmailCc + ";" + auto.EmailBcc));
                }
