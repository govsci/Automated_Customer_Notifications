...
{
    public class NAV_Order_Acknowledgement_New
    {
        private List<AutoResponder> autoResponders = new List<AutoResponder>();
        public NAV_Order_Acknowledgement_New()
        {
            GetReady();
            SendResponders();
        }
        ...
        public void GetReady()
        {
            ...
            List<OrderHeader> orders = Database.GetHeaderData("Order Ack Header");

            for (int i = 0; i < orders.Count; i++)
            {
                OrderHeader order = orders[i];
                Database.GetLineData(ref order, "Nav Order Ack Line");
        ...
        private void SendResponders()
        {
            foreach (AutoResponder auto in autoResponders)
            {
                ...
                {
                    Email.SendErrorMessage(ex, "NAV_Order_Acknowledgement_New", "SendResponders", new SqlCommand(auto.EmailTo + ";" + auto.EmailCc + ";" + auto.EmailBcc));
                }
